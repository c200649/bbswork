...
/* 期末課題　１） 投稿されたデータを投稿した新しい順に表示させる order by desc*/
    ...
    <div id="wrap">
        ...
        <div id="content">
            ...
            <form action="" method="post">
                <dl>
                    <?php /* ２）掲示板の投稿画面に、会員登録時に入力したニックネーム($User)を表示させる */ ?>
      ...
      ?>
            <div class="msg">
                <img src="member_picture/<?php echo hsc($post['picture']); ?>" width="48" height="48"
                    alt="<?php echo hsc($post['user']); ?>" />
                <p><?php echo makeLink(hsc($post['message'])); ?><span
                        class="name">（<?php echo hsc($post['user']); ?>）</span>[<a
                        href="index2.php?res=<?php echo hsc($post['id']); ?>">Re</a>]</p>
                <p class="day"><a
                        href="view.php?id=<?php echo hsc($post['id']); ?>"><?php echo hsc($post['created']); ?></a>
                    <?php
      ...
      ?>
        ...
        if ($page < $maxPage) {
        ?>
                <?php /* ３） 掲示板画面のページネーション($page)を完成させる */ ?>
